 ...
 *
 ****************************************************************************************
 THIS FILE HAS BEEN MODIFIED FOR THE PURPOSE CONFORMING TO MATLAB SYNTAX
...
(function (a) {
	...
	a.rfrac = a.mbase.Subclass({ ///////////////I added this!
			type: "rfrac",
			num: 1,
			den: 0,
			linebreakContainer: true,
...
(function (ac) {
	...
	var z = [{
			...
			input: "petzold", // THIS IS THE ∈ Element Symbol!!!!! I HAD TO GET RIDE OF IT!
			tag: "mo",
			output: "\u2208",
			tex: null,
			ttype: c
			...
			input: "cosd",
			tag: "mo",  // mo maybe a call to something else
			output: "cosd",
			tex: null,
			ttype: A,
			func: true	
			...
			input: "asin", //from arcsin to asin
			tag: "mo",
			output: "asin",
			tex: null,
			ttype: A,
			...
			input: "acos", //I am editing this from arcos to acos
			tag: "mo",
			output: "acos",
			tex: null,
			ttype: A,
	...
	function M(ae, ad) {
		...
		ae = ae.replace(/&gt;/g, ">");
		ae = ae.replace(/&lt;/g, "<");
	ae = ae.replace(/(Sin|Cos|Tan|Asin|Acos|Atan|Sinh|Cosh|Tanh|Cot|Sec|Csc|Log|Ln|Abs|Cosd|Sind|Tand|inv)/g, function (ag) {
...
(function (i, c, f, g) {
	...
	g.Augment({
		...
		Startup: function () {
			...
			var q = g.addElement(document.body, "mjx-block", {
					style: {
						display: "block",
						width: "5in"
					}
			...
			this.TestSpan = g.Element("mjx-test", {
					style: {
						left: "1em"
					}
				}, [["mjx-ex-box-test"]]);
		...
		postTranslate: function (v) {
			var r = v.jax[this.id];
			for (var t = v.CHTMLlast, q = v.CHTMLeqn; t < q; t++) {
				var s = r[t];
		...
		Zoom: function (s, z, y, q, w) {
			...
			var t = g.addElement(z, "mjx-chtml", {
					style: {
						"font-size": Math.floor(g.scale * 100) + "%"
					},
					isMathJax: false
				});
			g.CHTMLnode = t;
		...
		lookupChar: function (t, w) {
			var v = t;
			while (t) {
				for (var s = 0, q = t.fonts.length; s < q; s++) {
		...
		getHDW: function (z, s, D) {
			var r = g.addElement(g.CHTMLnode, "mjx-chartest", {
					className: s
				}, [["mjx-char", {
								style: D
							}, [z]]]);
			var q = g.addElement(g.CHTMLnode, "mjx-chartest", {
					className: s
				}, [["mjx-char", {
								style: D
							}, [z, ["mjx-box"]]]]);
		...
		createDelimiter: function (v, q, s, y, t) {
			...
			while (r.alias) {
				...
				if (!r) {
					r = {
						HW: [0, this.FONTDATA.VARIANT[h.VARIANT.NORMAL]]
					}
				}
			}
			if (r.load) {
			...
			for (var w = 0, u = r.HW.length; w < u; w++) {
		...
		createChar: function (s, w, u, r) {
			...
			if (w[0]instanceof Array) {
				for (var x = 0, t = w[0].length; x < t; x++) {
	...
	MathJax.Hub.Register.StartupHook("mml Jax Ready", function () {
		...
		h.mbase.Augment({
			...
			CHTMLcreateNode: function (q) {
				...
				return this.CHTMLhandleAttributes(g.addElement(q, "mjx-" + this.type, {
						id: r
					}))
			},
			...
			CHTMLhandleStyle: function (t) {
				...
				this.removedStyles = {};
				for (var r = 0, q = g.removeStyles.length; r < q; r++) {
			...
			CHTMLhandleBBox: function (u) {
				...
				for (var s = 0, q = g.BBOX.styleAdjust.length; s < q; s++) {
		...
		h.mo.Augment({
			toCommonHTML: function (t) {
				...
				if (r.text == "") {
					...
					for (var s = 0, q = this.data.length; s < q; s++) {
		...
		h.rfrac.Augment({
			toCommonHTML: function (M) {
				...
				var w = this.getValues("linethickness", "displaystyle", "numalign", "denomalign", "bevelled");
				var N = w.displaystyle;
				var C = M.lastChild, //Petzold! Changed it from s = M.lastChild
				s = M.firstChild; // Petzold changed it from C = M.firstChild
				var x = g.addElement(M, "mjx-box");
				x.appendChild(C);
				x.appendChild(s);
				M.appendChild(x);
				...
				if (w.bevelled) {
					...
					if (N) {
						...
						//PETZOLD changed this as well
						F = (I === 0 ? g.TEX.num3 : g.TEX.num2);
						G = g.TEX.denom2
					}
					if (I === 0) {
						K = Math.max((N ? 7 : 3) * g.TEX.rule_thickness, 2 * L);
		...
		*/
		h.rfrac.Augment({
			toCommonHTML: function (M) {
				...
				M.appendChild(x);
				if (w.numalign !== "center") {
					s.style.textAlign = w.numalign
				}
				if (w.denomalign !== "center") {
					C.style.textAlign = w.denomalign
				}
				var O = this.CHTMLbboxFor(0),
				A = this.CHTMLbboxFor(1),
				B = g.BBOX.empty(this.CHTML),
				...
				if (w.bevelled) {
					...
					F = y * (A.d - A.h) / 2 + S - R;
					if (G) {
						s.style.verticalAlign = g.Em(G / E)
					}
					if (F) {
						C.style.verticalAlign = g.Em(F / y)
					}
					Q.style.marginLeft = Q.style.marginRight = g.Em(-R / 2);
					B.combine(O, 0, G);
					B.combine(r, E * O.w - R / 2, 0);
					...
					B.clean();
					x.style.width = g.Em(B.w);
					s.style.width = g.Em(B.w / E);
					C.style.width = g.Em(B.w / y);
					if (z) {
						z.style.width = x.style.width
					}
					s.style.top = g.Em(-B.h / E);
					C.style.bottom = g.Em(-B.d / y);
					g.addElement(M, "mjx-vsize", {
						style: {
							height: g.Em(B.h + B.d),
							verticalAlign: g.Em(-B.d)
		...
		h.mrow.Augment({
			toCommonHTML: function (t) {
				...
				u = w.d,
				v;
				for (var r = 0, q = this.data.length; r < q; r++) {
		...
		h["annotation-xml"].Augment({
			toCommonHTML: h.mbase.CHTMLautoload
		});
		h.ms.Augment({
			toCommonHTML: h.mbase.CHTMLautoload
		});
	...
	b.mtable.Augment({
		...
		CHTMLgetBoxSizes: function (x, k) {
			...
			for (var p = 0, n = this.data.length; p < n; p++) {
				...
				w[p] = q;
				h[p] = r;
				for (var o = y, u = z.data.length + y; o < u; o++) {
					if (l[o] == null) {
						l[o] = -f;
						if (o > v) {
							v = o
		...
		CHTMLadjustCells: function (h, p) {
			...
			for (var I = 0, E = J.length; I < E; I++) {
				...
				for (var G = 0, w = t.length; G < w; G++) {
					...
					if (p.HD && G === 0) {
						e.addElement(t[G].parentNode, "mjx-mtd", {
							style: {
								padding: n + " 0 " + D
							}
						}, [["mjx-box", {
